...
Windows용 실행파일은 Windows 환경에서 `build.bat`을 더블클릭해 생성합니다. 생성 결과는 `releases\JiraSCCB.exe`에 저장되며, 사용자는 Python 설치 없이 이 파일을 더블클릭해 실행할 수 있습니다.

`build.bat`은 빌드용 스크립트이므로 실행파일을 만드는 PC에는 Python이 필요합니다. 일반 사용자는 생성된 `JiraSCCB.exe`만 더블클릭하면 됩니다.
=======
